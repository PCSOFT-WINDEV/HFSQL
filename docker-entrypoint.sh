...
if [ "$1" = '/opt/hfsql/manta64' ] || [ "$1" = 'createhfsqluser' ]; then
	if [ ! -d "/var/lib/hfsql/__system" ]; then
		...
		if [ -z "${HFSQL_PASSWORD-}" ] && [ -z "${HFSQL_ALLOW_EMPTY_PASSWORD-}" ] && [ -z "${HFSQL_RANDOM_PASSWORD-}" ]; then
			echo "You need to specify one of HFSQL_PASSWORD/HFSQL_PASSWORD_FILE, HFSQL_RANDOM_PASSWORD or HFSQL_ALLOW_EMPTY_PASSWORD"
			exit -1
		fi
		
		if [ -n "${HFSQL_RANDOM_PASSWORD-}" ]; then
			if [ -n "${HFSQL_PASSWORD}" ]; then
